...
public class Review {
	...
	//Default constructor used to make a review object, and retrieve all of its statistics which include likes and dislikes
	...
	/**
	 * Overloaded constructor to actually create a review object
	 * @param author
	 * @param id
	 */
	public Review(int id, String reviewContent, int stars) {
		try {
			int reviewCount = 0;
			Connection c = getConnection();
			PreparedStatement ptsmt = (PreparedStatement) c.prepareStatement("SELECT COUNT(restaurantID) as reviewCount from review where restaurantID = ?");
			ptsmt.setInt(1, id);
